...
router.get('/read/:tournoi_id', (req, res, next) => {
  const tournoi_id = req.params.tournoi_id;
  //var query = "select * from equipe_tournoi where tournoi_id=?";
  var query = "select equipe_tournoi.id, tournoi.nom AS Tournoi, categorie.nom AS Categorie, club.nom AS Club, equipe_tournoi.nb_equipe \
    from equipe_tournoi \
    ...
    where tournoi_id=?";
  connection.query(query, [tournoi_id], (err, results) => {
    if(!err){
